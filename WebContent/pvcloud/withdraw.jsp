...
<div id="wrapper">
	<div id="page-wrapper" class="gray-bg dashbard-1" style="background-color:#fff;margin-top:50px;">
	  ...
	  <%--   	<div class="fl"><img src="${CONTEXT_PATH }/image/picturesfolder.ico" style="width:50px; height:50px;"/></div> --%>
	   		<div style="font-size: 15px;color: #686868;font-family:Georgia, serif;text-align: left;padding-left: 20px;font-weight: bold;"><span class="glyphicon glyphicon-home"></span>&nbsp;提现申请信息</div>
	   </div>
    	<hr/>	
		    				...
		    				<tr class="bOrder">
		    					...
		    					<td>
		    							<c:if test="${s.statusCd=='190001'}">
		    									<input type="button" value="审核成功" class="btn btn-primary" onclick="if(confirm('确认要提交数据?')){window.location='${CONTEXT_PATH}/withdrawInfo/update?status=190002&id=${s.id}';}"/>
		    									<input type="button" value="审核失败" class="btn btn-primary" onclick="if(confirm('确认要提交数据?')){window.location='${CONTEXT_PATH}/withdrawInfo/update?status=190003&id=${s.id}';}"/>
		    							</c:if>
		    							<input type="button" value="查看用户" class="btn btn-primary" data-toggle="modal" data-target="#myModal1" onclick="loadProject1(${s.memberId})"/>
		    							<input type="button" value="编辑备注" class="btn btn-primary" data-toggle="modal" data-target="#myModal2" onclick="loadProject2(${s.id})"/>
		    					</td>
		    				</tr>
		    			</c:forEach>
					</c:if>
