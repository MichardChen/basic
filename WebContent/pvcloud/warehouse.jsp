...
<div id="wrapper">
	<div id="page-wrapper" class="gray-bg dashbard-1" style="background-color:#fff;margin-top:50px;">
	  ...
	  <%--   	<div class="fl"><img src="${CONTEXT_PATH }/image/picturesfolder.ico" style="width:50px; height:50px;"/></div> --%>
	   		<div style="font-size: 15px;color: #686868;font-family:Georgia, serif;text-align: left;padding-left: 20px;font-weight: bold;"><span class="glyphicon glyphicon-home"></span>&nbsp;仓库信息</div>
	   </div>
    	<hr/>	
    	<div class="span" style="width:100%;color:black;font-size:12px;border:2px solid #dadada;">
   		<div class="" style="margin-top:15px;margin-bottom:15px;">
    		<form method="post" action="${CONTEXT_PATH}/warehouseInfo/queryByPage" class="form-horizontal">
    			<div style="" class="form-group">
    				<label class="col-sm-1 col-xs-1 col-md-1 control-label">仓库名称</label>
    ...
    <div class="container equip" style="width:100%;font-size:12px;border:1px solid #dadada;margin-top:15px;height:700px;position:relative;color:black;margin-left:0px;">
    	<div class="row">
		    		...
		    		<c:if test="${list.totalRow>0 }">
		    			<c:forEach var="s" items="${sList}" varStatus="status">	
		    				<tr class="bOrder">
		    					...
		    					<td>
		    							<c:if test="${s.flg==1}">正常</c:if>
		    							<c:if test="${s.flg==0}">删除</c:if>
		    					</td>
		    					<td>
		    							<c:if test="${s.flg==1}">
		    									<input type="button" value="删除" class="btn btn-default" onclick="if(confirm('确认要删除数据?')){window.location='${CONTEXT_PATH}/warehouseInfo/del?id=${s.id}';}"/>
		    									<input type="button" value="编辑" class="btn btn-primary" data-toggle="modal" data-target="#myModal1" onclick="edit(${s.id})"/>
		    							</c:if>
		    					</td>
		    				</tr>
		    			</c:forEach>
    		...
    		<div id="botton" style="position:absolute;bottom:50px;right:5%;color:black;margin:0 auto;font-size:12px;">
					...
					<c:set var="pageUrl" scope="request" value="${CONTEXT_PATH}/warehouseInfo/queryByConditionByPage/-" />    	
