...
<div id="wrapper">
	<div id="page-wrapper" class="gray-bg dashbard-1" style="background-color:#fff;margin-top:50px;text-align: center;">
	  ...
	  <%--   	<div class="fl"><img src="${CONTEXT_PATH }/image/picturesfolder.ico" style="width:50px; height:50px;"/></div> --%>
	   		<div style="font-size: 15px;color: #686868;font-family:Georgia, serif;text-align: left;padding-left: 20px;font-weight: bold;"><span class="glyphicon glyphicon-home"></span>&nbsp;用户信息</div>
	   </div>
    	<hr/>	
		    		...
		    		<c:if test="${list.totalRow>0 }">
		    			<c:forEach var="s" items="${sList}" varStatus="status">	
		    				<tr class="bOrder">
		    					...
		    					<td>
		    						<input type="button" value="编辑" class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="loadProject(${s.id})"/>
		    						...
		    						<c:if test="${s.roleCd=='350002' and s.openStore == 0}">
		    							<input type="button" value="增加门店" class="btn btn-primary" data-toggle="modal" data-target="#myModal3" onclick="openStore1(${s.id})"/>
		    						</c:if>
		    						<c:if test="${s.openStore == 1 and s.roleCd=='350002'}">
		    							<input type="button" value="查看门店" class="btn btn-primary" data-toggle="modal" data-target="#myModal4" onclick="openStore2(${s.storeId})"/>
		    						</c:if>
		    						<c:if test="${s.roleCd=='350001'}">
		    								<input type="button" value="设置为经销商" class="btn btn-primary"  style="width: 100px;" onclick="if(confirm('确认要设置经销商?')){window.location='${CONTEXT_PATH}/memberInfo/updateBusiness?id=${s.id}';}"/>
		    						</c:if>
		    					</td>
		    				</tr>
		    			</c:forEach>
