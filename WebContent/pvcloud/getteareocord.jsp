...
function exportData(){
	if(confirm('确认要导出数据?')){
		...
		var time1 = $("#time1").val();
		var time2 = $("#time2").val();
		var params = "?status="+status+"&mobile="+mobile+"&time1="+time1+"&time2="+time2;
		window.location.href="${CONTEXT_PATH}/getTeaRecordInfo/exportData"+params;
	}else{
		return false;
	}
}
...
<div id="wrapper">
	<div id="page-wrapper" class="gray-bg dashbard-1" style="background-color:#fff;margin-top:50px;">
	  ...
	  <%--   	<div class="fl"><img src="${CONTEXT_PATH }/image/picturesfolder.ico" style="width:50px; height:50px;"/></div> --%>
	   		<div style="font-size: 15px;color: #686868;font-family:Georgia, serif;text-align: left;padding-left: 20px;font-weight: bold;"><span class="glyphicon glyphicon-home"></span>&nbsp;取茶信息</div>
	   </div>
    	<hr/>	
	...
	<div class="span" style="width:100%;color:black;font-size:12px;border:2px solid #dadada;">
   		<div class="" style="margin-top:15px;margin-bottom:15px;">
    		<form method="post" action="${CONTEXT_PATH}/getTeaRecordInfo/queryByConditionByPage" class="form-horizontal">
    			<div style="" class="form-group">
    				<label class="col-sm-1 col-xs-1 col-md-1 control-label">申请时间</label>
	    			<div class="col-sm-2 col-xs-2 col-md-2">	
	    				<input type="text" class="form-control" name="time1" id="time1" value="${time1}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})"/>
    				</div>
    				<div class="col-sm-2 col-xs-2 col-md-2">	
	    				<input type="text" class="form-control" name="time2" id="time2" value="${time2}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})"/>
    				</div>
    ...
    <div class="container equip" style="width:100%;font-size:12px;border:1px solid #dadada;margin-top:15px;height:700px;position:relative;color:black;margin-left:0px;">
    	<div class="row">
    		...
    		<div id="botton" style="position:absolute;bottom:50px;right:5%;color:black;margin:0 auto;font-size:12px;">
					...
					<c:set var="pageUrl" scope="request" value="${CONTEXT_PATH}/getTeaRecordInfo/queryByPage/-" />    	
