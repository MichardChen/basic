...
@TableBind(table = "mk_vertify_code", pk = "id")
public class VertifyCode extends Model<VertifyCode> {
	...
	public VertifyCode queryVertifyCode(String mobile){
		return VertifyCode.dao.findFirst("select * from mk_vertify_code where mobile=? order by create_time desc limit 1", mobile);
	}
	
	public void updateVertifyCode(String mobile,String code){
		Db.update("update mk_vertify_code set code='"+code+"',expire_time='"+DateUtil.getVertifyCodeExpireTime()+"',update_time='"+DateUtil.getNowTimestamp()+"' where mobile="+mobile);
	}
	
	public void updateVertifyCodeExpire(String mobile,Timestamp expireTime){
		Db.update("update mk_vertify_code set expire_time='"+expireTime+"' where mobile="+mobile);
	}
}
